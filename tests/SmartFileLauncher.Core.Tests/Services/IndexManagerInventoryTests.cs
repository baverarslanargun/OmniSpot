...
{
    [Fact]
    public async Task InventoryProgressDistinguishesUnknownTotalsAndCompletedPersistence()
    {
        using var world = new World();
        var path = world.Workspace.CreateFile(@"root\file.txt", "x");
        var progress = new System.Collections.Concurrent.ConcurrentQueue<IndexProgress>();
        var ready = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        world.Manager.OnProgress += update => {
            progress.Enqueue(update);
            if (update.Phase == "ready") ready.TrySetResult();
        };
        await world.Initialize(new Source((receive, _) => {
            receive(Entry(world.Root, true));
            receive(Entry(path));
            return Task.CompletedTask;
        }));
        await ready.Task.WaitAsync(TimeSpan.FromSeconds(5));
        Assert.All(progress.Where(update => update.Phase == "inventory"), update => Assert.True(update.IsIndeterminate));
        Assert.Contains(progress, update => update.Phase == "inventory");
        Assert.Contains(progress, update => update.Phase == "persist" && update.Percentage == 0);
        Assert.Contains(progress, update => update.Phase == "persist" && update.Percentage == 100 &&
            update.ItemCount == 2 && update.TotalItemCount == 2 && !update.IsIndeterminate);
        Assert.NotNull(world.Database.GetFileByPath(path));
    }
