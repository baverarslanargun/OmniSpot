...
internal sealed partial class LiveCatalog
{
    ...
    private sealed class SealedReader : CatalogReader, IDirectCatalogMatches
    {
        ...
        internal override int TokenCount => _stamp.Terms;
        ...
        internal override IEnumerable<int> Roots() { yield return 0; }
        ...
        internal override bool ContainsToken(string token) => Keep(_owner.FindTerm(token) != 0);
        ...
        internal override IEnumerable<int> Children(string path, Dictionary<int, string>? pathCache = null)
        {
            try
            {
                ...
                if (parent != 0) foreach (var node in _owner.ChildIds(parent)) yield return node - 1;
            }
            finally { GC.KeepAlive(this); }
        }
